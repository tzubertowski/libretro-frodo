...
int Retro_PollEvent(uint8 *key_matrix, uint8 *rev_matrix, uint8 *joystick)
{
	...
	if (pauseg == 0)
	{
		...
		if (mbt[i] == 0 && input_state_cb(0, RETRO_DEVICE_JOYPAD, 0, i))
			mbt[i] = 1;
		else if (mbt[i] == 1 && !input_state_cb(0, RETRO_DEVICE_JOYPAD, 0, i))
		{
			mbt[i] = 0;
...
int Retro_PollEvent(uint8 *key_matrix, uint8 *rev_matrix, uint8 *joystick)
{
	...
	// if (input_state_cb(0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_X) && shiftstate == 0)
	// {
	// 	mbt[RETRO_DEVICE_ID_JOYPAD_X]++;
	// 	if (mbt[RETRO_DEVICE_ID_JOYPAD_X] > 2)
	// 	{
	// 		kbd_buf_feed((char*)" ");
	// 		mbt[RETRO_DEVICE_ID_JOYPAD_X] = 0; // Reset to prevent repeated triggers
	// 	}
	// }
	// else
	// {
	// 	mbt[RETRO_DEVICE_ID_JOYPAD_X] = 0;
	// }
	...
	if (SHOWKEY != 1)
	{
		...
		else if (shiftstate != 0)
		{
			...
			if (shifted_cursor[0] != 0)
				validkey(MATRIX(0, 7) | 0x80, 1, key_matrix, rev_matrix, joystick);
			if (shifted_cursor[1] != 0)
				validkey(MATRIX(0, 7), 1, key_matrix, rev_matrix, joystick);
			if (shifted_cursor[2] != 0)
				validkey(MATRIX(0, 2) | 0x80, 1, key_matrix, rev_matrix, joystick);
			if (shifted_cursor[3] != 0)
				validkey(MATRIX(0, 2), 1, key_matrix, rev_matrix, joystick);
			if (shifted_cursor[4] != 0)
				validkey(MATRIX(0, 4), 1, key_matrix, rev_matrix, joystick);
			if (shifted_cursor[5] != 0)
				validkey(MATRIX(0, 5), 1, key_matrix, rev_matrix, joystick);
			if (shifted_cursor[6] != 0)
				validkey(MATRIX(0, 6), 1, key_matrix, rev_matrix, joystick);
