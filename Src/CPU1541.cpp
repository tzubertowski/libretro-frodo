...
void MOS6502_1541::do_adc(uint8 byte)
{
   ...
   else
   {
      uint16 ah;
      // Decimal mode
      uint16 al = (a & 0x0f) + (byte & 0x0f) + (c_flag ? 1 : 0);	// Calculate lower nybble
      if (al > 9)
...
void MOS6502_1541::do_sbc(uint8 byte)
{
   ...
   {
      // Decimal mode
      uint16 al = (a & 0x0f) - (byte & 0x0f) - (c_flag ? 0 : 1);		// Calculate lower nybble
      uint16 ah = (a >> 4) - (byte >> 4);							         // Calculate upper nybble
      if (al & 0x10)
      {
         al -= 6;											      // BCD fixup for lower nybble
         ah--;
...
void MOS6502_1541::SetState(MOS6502State *s)
{
	...
	via1_pra  = s->via1_pra;
   via1_ddra = s->via1_ddra;
	via1_prb  = s->via1_prb;
   via1_ddrb = s->via1_ddrb;
	via1_t1c  = s->via1_t1c;
   via1_t1l  = s->via1_t1l;
	via1_t2c  = s->via1_t2c;
   via1_t2l  = s->via1_t2l;
   ...
   via1_ier  = s->via1_ier;
	...
	via2_pra  = s->via2_pra;
   via2_ddra = s->via2_ddra;
	via2_prb  = s->via2_prb;
   via2_ddrb = s->via2_ddrb;
	via2_t1c  = s->via2_t1c;
   via2_t1l  = s->via2_t1l;
	via2_t2c  = s->via2_t2c;
   via2_t2l  = s->via2_t2l;
